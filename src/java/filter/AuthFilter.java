...
public class AuthFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        if (session == null || (session.getAttribute("infoCustomer") == null && session.getAttribute("accountCustomer") == null)) {
            Cookie[] cookies = req.getCookies();
            // Kiểm tra cookies có tồn tại ko?
            for (Cookie cooky : cookies) {
                if (cooky.equals("auth-token")) {
                    String token = cooky.getValue();
                    email = TokenDAO.getUsernameByToken(token);
                    if (!email.isEmpty()) {
